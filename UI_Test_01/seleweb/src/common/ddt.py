...
index_len = 5              # default max length of testcase index
...
def file_data(value):
    ...
    In testcase of a list, each value in the list will correspond to one
    test testcase, and the value will be concatenated to the test method
    name.

    In testcase of a dict, keys will be used as suffixes to the name of the
    test testcase, and values will be fed as test data.
...
def mk_test_name(name, value, index=0):
    """
    Generate a new name for a test testcase.

    It will take the original test name and append an ordinal index and a
    string representation of the value, and convert the reports into a valid
...
def add_test(cls, test_name, test_docstring, func, *args, **kwargs):
    """
    Add a test testcase to this class.
...
def ddt(cls):
    ...
    Apply this decorator to the test testcase class, and then
    ...
    For each method decorated with ``@file_data('datas_file.json')``, the
    decorator will try to load the datas_file.json file located relative
